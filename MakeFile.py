...
@click.option('--deploy', is_flag = True,
              help = "Deploy the app to your android device")
@click.option('--po', is_flag = True,
              help = "Create i18n message files")
...
def cli(test,coverage,docs,apk,deploy,po,mo):
    ...
    if po:
        ...
        subprocess.run("xgettext -Lpython --output=messages.pot src/waclient/waclient_app.py src/waclient/*.kv",
